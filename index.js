import{a as h,i as n}from"./assets/vendor-CCIwLLD6.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const r of a)if(r.type==="childList")for(const m of r.addedNodes)m.tagName==="LINK"&&m.rel==="modulepreload"&&o(m)}).observe(document,{childList:!0,subtree:!0});function s(a){const r={};return a.integrity&&(r.integrity=a.integrity),a.referrerPolicy&&(r.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?r.credentials="include":a.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function o(a){if(a.ep)return;a.ep=!0;const r=s(a);fetch(a.href,r)}})();const v=h.create({baseURL:"https://sound-wave.b.goit.study/api/artists",headers:{},params:{limit:8}});async function f(t){return(await v.get("",{params:{page:t}})).data}let l,d;document.addEventListener("DOMContentLoaded",y);async function y(t){t.preventDefault(),l=1;try{const e=await f(l);if(d=Math.ceil(e.totalArtists/8),e.artists.length===0){n.show({title:"Oops",message:"The are no artists found..."}),i();return}else L(e.artists),i(),g()}catch{d=0,n.show({title:"Oops",message:"An error must have occurred"}),i()}}function g(){if(d>l)C();else if(l===d){A(),n.show({title:"Wow",message:"You have reached the end of the list"});return}}function L(t){const e=document.querySelector(".artists-list-js"),s=t.map(o=>w(o)).join("");e.innerHTML=s}function w(t){const e=t.genres.map(o=>b(o)).join("");let s;return t.strBiographyEN.length>120?s=t.strBiographyEN.slice(0,120)+"...":s=t.strBiographyEN,` <li class="artists-list-item">
  ...
  <p>${t}</p>
  </li>`}function $(t){const e=document.querySelector(".artists-list-js"),s=t.map(o=>w(o)).join("");e.insertAdjacentHTML("beforeend",s)}function j(){const t=document.querySelector(".artist-loader-js");t.style.display="flex"}function i(){const t=document.querySelector(".artist-loader-js");t.style.display="none"}function C(){document.querySelector(".artists-load-js").classList.add("artists-load-more")}function A(){document.querySelector(".artists-load-js").classList.remove("artists-load-more")}const M=document.querySelector(".artists-load-js");M.addEventListener("click",S);async function S(t){t.preventDefault(),l+=1,j();try{const e=await f(l);i(),$(e.artists),g()}catch{n.show({title:"Oops",message:"An error must have occurred"}),i()}}console.log("Im feedback");document.querySelectorAll(".nav-list a").forEach(t=>{t.addEventListener("click",()=>{document.getElementById("menu-toggle").checked=!1})});const p={artistsList:document.querySelector(".artists-list-js")};q();function q(){let t;p.artistsList.addEventListener("click",async function(e){const s=e.target.closest(".artists-learn-js");if(s&&p.artistsList.contains(s)){t=s.dataset.artistId;try{P();const o=await h.get(`https://sound-wave.b.goit.study/api/artists/${t}/albums`);u(),W(),k(o.data),T(o.data),O()}catch{u(),c(),n.show({title:"Oops",message:"An error must have occurred"})}}})}function k(t){const e=document.querySelector(".modal-window-js"),s=t.genres.map(a=>x(a)).join(""),o=`<div class="modal-content">
  ...
  </div>`;e.innerHTML=o,u()}function x(t){return`
  <li class="artist-genre">
  <p>${t}</p>
  </li>`}function E(t){let e;t.movie?e=`<a href="${t.movie}" target="_blank"> <svg width="21" height="15" viewBox="0 0 21 15" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  </tr> `}function B(t){const e=(t.tracks||[]).map(s=>E(s)).join("");return`
